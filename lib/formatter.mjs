import chalk from 'chalk';
import { diffJson } from 'diff';

export function formatDiff(diff, format = 'pretty') {
  if (format === 'json') {
    return formatJsonOutput(diff);
  }
  
  if (format === 'side-by-side') {
    if (Array.isArray(diff)) {
      return formatDocumentDiffSideBySide(diff);
    }
    return formatQueryDiffSideBySide(diff);
  }
  
  if (Array.isArray(diff)) {
    return formatDocumentDiff(diff);
  }
  
  return formatQueryDiff(diff);
}

function formatDocumentDiff(differences) {
  if (differences.length === 0) {
    return chalk.green('✓ Documents are identical');
  }

  let output = chalk.yellow('\n⚠ Document differences found:\n\n');

  for (const diff of differences) {
    output += formatSingleDiff(diff) + '\n';
  }

  return output;
}

function formatQueryDiff(diff) {
  const { added, removed, changed } = diff;
  let output = '';

  if (added.length === 0 && removed.length === 0 && changed.length === 0) {
    return chalk.green('✓ Query results are identical');
  }

  output += chalk.yellow('\n⚠ Query differences found:\n');

  if (removed.length > 0) {
    output += chalk.red(`\n━━━ Removed (${removed.length} documents) ━━━\n`);
    for (const item of removed) {
      output += chalk.red(`- [${item.key}]`) + '\n';
      output += formatValue(item.document, '  ', chalk.red) + '\n';
    }
  }

  if (added.length > 0) {
    output += chalk.green(`\n━━━ Added (${added.length} documents) ━━━\n`);
    for (const item of added) {
      output += chalk.green(`+ [${item.key}]`) + '\n';
      output += formatValue(item.document, '  ', chalk.green) + '\n';
    }
  }

  if (changed.length > 0) {
    output += chalk.blue(`\n━━━ Changed (${changed.length} documents) ━━━\n`);
    for (const item of changed) {
      output += chalk.blue(`~ [${item.key}]`) + '\n';
      for (const diff of item.diff) {
        output += '  ' + formatSingleDiff(diff) + '\n';
      }
      output += '\n';
    }
  }

  return output;
}

function formatSingleDiff(diff) {
  const path = chalk.gray(diff.path || 'root');

  switch (diff.type) {
    case 'added':
      return `${chalk.green('+')} ${path}: ${formatValue(diff.value, '', chalk.green)}`;
    
    case 'removed':
      return `${chalk.red('-')} ${path}: ${formatValue(diff.value, '', chalk.red)}`;
    
    case 'changed':
      return `${chalk.blue('~')} ${path}:\n` +
             `    ${chalk.red('- ' + formatValue(diff.oldValue))}\n` +
             `    ${chalk.green('+ ' + formatValue(diff.newValue))}`;
    
    default:
      return '';
  }
}

function formatValue(value, indent = '', colorFn = null) {
  if (value === null) return colorFn ? colorFn('null') : 'null';
  if (value === undefined) return colorFn ? colorFn('undefined') : 'undefined';

  if (typeof value === 'string') {
    const formatted = JSON.stringify(value);
    return colorFn ? colorFn(formatted) : formatted;
  }

  if (typeof value === 'number' || typeof value === 'boolean') {
    const formatted = String(value);
    return colorFn ? colorFn(formatted) : formatted;
  }

  if (value._type) {
    const formatted = formatSpecialType(value);
    return colorFn ? colorFn(formatted) : formatted;
  }

  if (Array.isArray(value)) {
    if (value.length === 0) {
      return colorFn ? colorFn('[]') : '[]';
    }
    
    let result = '[\n';
    for (let i = 0; i < value.length; i++) {
      result += indent + '  ' + formatValue(value[i], indent + '  ', colorFn);
      if (i < value.length - 1) result += ',';
      result += '\n';
    }
    result += indent + ']';
    return result;
  }

  if (typeof value === 'object') {
    const keys = Object.keys(value);
    if (keys.length === 0) {
      return colorFn ? colorFn('{}') : '{}';
    }

    let result = '{\n';
    for (let i = 0; i < keys.length; i++) {
      const key = keys[i];
      const formattedKey = colorFn ? colorFn(`"${key}"`) : `"${key}"`;
      result += `${indent}  ${formattedKey}: ${formatValue(value[key], indent + '  ', colorFn)}`;
      if (i < keys.length - 1) result += ',';
      result += '\n';
    }
    result += indent + '}';
    return result;
  }

  return String(value);
}

function formatSpecialType(value) {
  switch (value._type) {
    case 'Timestamp':
      return `Timestamp(${value.iso})`;
    
    case 'GeoPoint':
      return `GeoPoint(${value.latitude}, ${value.longitude})`;
    
    case 'DocumentReference':
      return `DocumentReference(${value.path})`;
    
    case 'Bytes':
      return `Bytes(${value.length} bytes)`;
    
    case 'Date':
      return `Date(${value.iso})`;
    
    default:
      return JSON.stringify(value);
  }
}

function formatJsonOutput(diff) {
  if (Array.isArray(diff)) {
    return {
      type: 'document',
      hasDifferences: diff.length > 0,
      differences: diff
    };
  }

  return {
    type: 'query',
    hasDifferences: diff.added.length > 0 || diff.removed.length > 0 || diff.changed.length > 0,
    summary: {
      added: diff.added.length,
      removed: diff.removed.length,
      changed: diff.changed.length
    },
    differences: diff
  };
}

function formatDocumentDiffSideBySide(differences) {
  if (differences.length === 0) {
    return chalk.green('✓ Documents are identical');
  }

  const terminalWidth = process.stdout.columns || 120;
  const columnWidth = Math.floor((terminalWidth - 5) / 2); // Account for separator
  
  let output = chalk.yellow('\n⚠ Document differences found:\n\n');
  output += formatSideBySideHeader(columnWidth);
  output += '\n';

  // Group changes by path for side-by-side display
  const changesByPath = new Map();
  
  for (const diff of differences) {
    if (!changesByPath.has(diff.path)) {
      changesByPath.set(diff.path, { before: null, after: null, type: diff.type });
    }
    
    const change = changesByPath.get(diff.path);
    if (diff.type === 'removed' || diff.type === 'changed') {
      change.before = diff.type === 'changed' ? diff.oldValue : diff.value;
    }
    if (diff.type === 'added' || diff.type === 'changed') {
      change.after = diff.type === 'changed' ? diff.newValue : diff.value;
    }
  }

  for (const [path, change] of changesByPath) {
    output += formatSideBySideChange(path, change, columnWidth) + '\n';
  }

  return output;
}

function formatQueryDiffSideBySide(diff) {
  const { added, removed, changed } = diff;
  
  if (added.length === 0 && removed.length === 0 && changed.length === 0) {
    return chalk.green('✓ Query results are identical');
  }

  const terminalWidth = process.stdout.columns || 120;
  const columnWidth = Math.floor((terminalWidth - 5) / 2);
  
  let output = chalk.yellow('\n⚠ Query differences found:\n\n');
  output += formatSideBySideHeader(columnWidth);
  output += '\n';

  // Show removed documents
  for (const item of removed) {
    output += formatSideBySideDocument(item.key, item.document, null, columnWidth) + '\n';
  }

  // Show added documents
  for (const item of added) {
    output += formatSideBySideDocument(item.key, null, item.document, columnWidth) + '\n';
  }

  // Show changed documents
  for (const item of changed) {
    output += formatSideBySideDocument(item.key, item.documentA, item.documentB, columnWidth) + '\n';
  }

  return output;
}

function formatSideBySideHeader(columnWidth) {
  const leftHeader = chalk.red('SOURCE A').padEnd(columnWidth);
  const rightHeader = chalk.green('SOURCE B').padEnd(columnWidth);
  const separator = chalk.gray('│');
  
  return leftHeader + ' ' + separator + ' ' + rightHeader + '\n' +
         chalk.gray('─'.repeat(columnWidth)) + ' ' + separator + ' ' + chalk.gray('─'.repeat(columnWidth));
}

function formatSideBySideChange(path, change, columnWidth) {
  const pathLine = chalk.blue(`[${path}]`);
  
  const leftValue = change.before !== null ? formatCompactValue(change.before) : '';
  const rightValue = change.after !== null ? formatCompactValue(change.after) : '';
  
  const leftFormatted = chalk.red(truncateText(leftValue, columnWidth));
  const rightFormatted = chalk.green(truncateText(rightValue, columnWidth));
  
  return pathLine + '\n' + 
         leftFormatted.padEnd(columnWidth + 10) + ' │ ' + rightFormatted; // +10 for ANSI codes
}

function formatSideBySideDocument(key, docA, docB, columnWidth) {
  const keyLine = chalk.blue(`[${key}]`);
  
  const leftValue = docA ? formatCompactValue(docA) : '';
  const rightValue = docB ? formatCompactValue(docB) : '';
  
  const leftFormatted = chalk.red(truncateText(leftValue, columnWidth));
  const rightFormatted = chalk.green(truncateText(rightValue, columnWidth));
  
  return keyLine + '\n' + 
         leftFormatted.padEnd(columnWidth + 10) + ' │ ' + rightFormatted;
}

function formatCompactValue(value, maxDepth = 2, currentDepth = 0) {
  if (value === null) return 'null';
  if (value === undefined) return 'undefined';
  
  if (typeof value === 'string') {
    return JSON.stringify(value);
  }
  
  if (typeof value === 'number' || typeof value === 'boolean') {
    return String(value);
  }
  
  if (value._type) {
    return formatSpecialType(value);
  }
  
  if (Array.isArray(value)) {
    if (currentDepth >= maxDepth) return `[${value.length} items]`;
    const items = value.slice(0, 3).map(item => formatCompactValue(item, maxDepth, currentDepth + 1));
    const suffix = value.length > 3 ? '...' : '';
    return `[${items.join(', ')}${suffix}]`;
  }
  
  if (typeof value === 'object') {
    if (currentDepth >= maxDepth) {
      const keys = Object.keys(value);
      return `{${keys.length} fields}`;
    }
    
    const entries = Object.entries(value)
      .slice(0, 3)
      .map(([k, v]) => `${k}: ${formatCompactValue(v, maxDepth, currentDepth + 1)}`);
    
    const suffix = Object.keys(value).length > 3 ? '...' : '';
    return `{${entries.join(', ')}${suffix}}`;
  }
  
  return String(value);
}

function truncateText(text, maxLength) {
  if (text.length <= maxLength) return text;
  return text.substring(0, maxLength - 3) + '...';
}

export function formatDetailedDiff(objA, objB) {
  const diff = diffJson(objA || {}, objB || {}, {
    ignoreWhitespace: false
  });

  let output = '';
  
  for (const part of diff) {
    if (part.added) {
      output += chalk.green(part.value);
    } else if (part.removed) {
      output += chalk.red(part.value);
    } else {
      output += chalk.gray(part.value);
    }
  }

  return output;
}